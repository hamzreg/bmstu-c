...
int fill_array(FILE *in_file, int *const first,
int *const last)
{
    ...
    int number;
    int *array_pointer = first;

    for ( ; array_pointer != last; array_pointer++)
    {
...
void even_passage(char *start, char *end, size_t size,
int (*compar)(const void *, const void *))
{
    char *array_pointer = end;

    for ( ; array_pointer > start; array_pointer -= size)
        if (compar(array_pointer, array_pointer - size) <= 0)
            swap(size, array_pointer, array_pointer - size);
}


void odd_passage(char *start, char *end, size_t size,
int (*compar)(const void *, const void *))
{
    char *array_pointer = start;

    for ( ; array_pointer < end; array_pointer += size)
        if (compar(array_pointer, array_pointer + size) >= 0)
            swap(size, array_pointer, array_pointer + size);
}
...
int **pe_dst)
{
    ...
    int filter_len = 0;
    const int *array_pointer = pb_src;

    for ( ; array_pointer < pe_src - 1; array_pointer++)
    {
...
void write_array_file(FILE *out_file, const int *const first,
