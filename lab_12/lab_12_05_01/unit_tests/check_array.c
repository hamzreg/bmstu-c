...
int compare_arrays(int *first, int *second, int first_size,
int second_size)
{
...
START_TEST(test_mysort_usual_array)
{
    ...
    compare_arrays(final_array, array, sizeof(array) / sizeof(array[0]), 10);
}
END_TEST
...
START_TEST(test_key_usual)
{
    ...
    compare_arrays(final_array, new_array, sizeof(final_array) / sizeof(final_array[0]), 3);
    free(new_array);
}
END_TEST
